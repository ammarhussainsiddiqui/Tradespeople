...
const LeadDetailsMob = ({ leadData, componentKey }) => {
    ...
    try {
      const response = await fetch("/api/get-reviewss", {
        ...
        body: JSON.stringify({
          ...
          tradepersonId: tradeperson.id, // Tradeperson ID
          jobId: leadData.id,
        }),
      });
      ...
      const hasTradeperson = data.reviews?.some(
        (review) => review.reviewType === "tradeperson"
      );
          ...
          <div className="flex flex-col flex-1 md:justify-between">
            <div>
              <h1 className="text-xl font-semibold  text-foreground break-words whitespace-normal  md:max-w-64">
                {leadData?.service?.type || "Unknown !"}
              </h1>
            </div>
            <div className="flex">
              {leadData?.user && (
                <div className="text-sm text-muted-foreground mt-0">
                  By {leadData?.user.firstName || "No User DetailProvided"}.
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col py-4">
              <p className="text-sm text-foreground font-bold">
                {!leadData?.user?.email ||
                  leadData?.user?.email == undefined ? (
                  "No Email Provided"
                ) : (
                  //<i>{leadData?.user.email}</i>
                  <i>

                    <div className="flex items-center gap-2 mb-1 group">
                      <a
                        href={`mailto:${leadData?.user?.email}?subject=${leadData?.service?.type}%20Assistance%20Inquiry&body=Hey%20there,%20Iâ€™ve%20just%20seen%20your%20job%20request%20that%20was%20posted%20on%20the%20TradeCore%20website.%20If%20you%20still%20need%20this%20job%20to%20complete,%20I%20can%20come%20and%20quote%20at%20a%20time%20that%20is%20convenient%20to%20you.%20I%20look%20forward%20to%20hearing%20from%20you.%0AThanks!`}
                ...
                {!leadData?.user?.phone ||
                  ...
                  <i>
                    ...
                    <div className="flex items-center gap-2 mb-1 group">
                      ...
                      <a
                        href={`https://wa.me/${leadData?.user?.phone}?text=Hey%20there,%20Iâ€™ve%20just%20seen%20your%20job%20request%20that%20was%20posted%20on%20the%20TradeCore%20website.%20If%20you%20still%20need%20this%20job%20to%20complete,%20I%20can%20come%20and%20quote%20at%20a%20time%20that%20is%20convenient%20to%20you.%20I%20look%20forward%20to%20hearing%20from%20you.%0AThanks!`}
            ...
            <div className="mt-2 md:mt-4 flex items-center flex-wrap space-x-2 sm:space-x-3 text-foreground">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-xs sm:text-sm">
                  {leadData?.postcode?.toUpperCase() || "No Postcode Provided"}
                </span>
              </div>
              <div className="flex items-center space-x-1">
              ...
              {reviews.length > 0 ? (
                <div className="overflow-y-auto max-h-64 w-full">
                  <div className="flex justify-between pb-2">
                    ...
                    {!hasTradepersonReview && (
                      <ReviewDialog requestId={requestId} setDialogUpdated={setDialogUpdated} />
                    )}
                  </div>
                  {reviews.map((review) => (
                    <div key={review.id} className="w-full">
                      <div
                        className={`flex items-start w-5/6 bg-muted p-4 rounded-md shadow-md mb-4  border ${review.reviewType === 'user' ? 'border-neutral-400 shadow-neutral-100 mr-auto' : ' border-accent shadow-accent ml-auto '
                          }`}
                      >
                            ...
                            {errorMessage ? (
                              ...
                              <div className="mt-4">
                                <p className="text-sm text-muted-foreground">
                                  Click on ask for closure to get reviews on this
                                  job.
                                </p>
                                <button
