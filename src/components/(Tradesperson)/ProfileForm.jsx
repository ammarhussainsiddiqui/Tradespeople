...
import userimg from '../../app/assets/userImage.webp'
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      if (file.size > maxImageSize) {
        toast.warning('Max image size limit is 2MB.', {
          position: "top-center",
        });
        return;
      }
    ...
    <div>
      <div className="flex items-center mb-6">
        <button type="button" onClick={handleImageClick} className="focus:outline-none">
          <Image
            src={image}
            alt="Profile Picture"
            className="rounded-full h-20 w-20 border-2 border-accent"
            width={80}
            height={80}
          />
        </button>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              className="mt-1 block w-full bg-neutral-100 py-4 px-3 text-xs border border-neutral-50 rounded-lg focus:ring-accent focus:border-accent"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          <div>
            <label className="block font-semibold text-sm font-medium text-ink-muted">Email</label>
            <input
              type="email"
              name="email"
              value={email}
          ...
          <div>
            <label className="block font-semibold text-sm font-medium text-ink-muted">Phone Number</label>
            <input
              type="text"
              name="phoneNumber"
              value={phone ? phone : ''}
        ...
        <div className="mt-6">
          <button
            type="submit"
            className="px-4 py-2 bg-accent hover:bg-primary hover:text-ink-inverse text-accent-foreground font-semibold rounded-md shadow-sm"
            disabled={isLoading}
          >
